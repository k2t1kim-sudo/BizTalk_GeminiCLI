...
- [ ] 프론트엔드 `index.html` 페이지가 FastAPI 서버에서 요청되도록 static routing 기능이 있어야 한다
...
5. `requirements.txt` 라이브러리의 버전을 명시하고 최신버전을 사용
...
4. Playerwright MCP를 사용한 브라우저 테스트
...
> 오늘 만든 구조가 그 출발점이 됩니다.�든 구조가 그 출발점이 됩니다.